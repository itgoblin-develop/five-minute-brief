...
| FR-13 | 비밀번호 찾기        | 이메일 인증 후 재설정                              | 🟠       | ✅ 백엔드 + 프론트 완료 (forgot-password/reset-password) |
...
| FR-24 | 새로고침         | Pull to refresh 기능                           | 🟠       | ✅ 프론트 완료 (리스트 뷰)  |
...
| FR-42 | 수동 알림 설정        | 사용자가 직접 시간/요일 설정         | 🔴       | ✅ 프론트 + 백엔드 완료 (GET/PUT /api/user/settings) |
| FR-43 | 푸시 on/off           | 알림 허용 여부 토글                  | 🔴       | ✅ 프론트 + 백엔드 완료 (GET/PUT /api/user/settings) |
| FR-44 | 알림 메시지           | "오늘의 뉴스가 도착했습니다" + 제목  | 🔴       | ⬜ 미구현                  |
| FR-45 | 알림 클릭 시 이동     | 앱/웹 메인 피드 이동                 | 🔴       | ⬜ 미구현                  |
| FR-46 | 디바이스 토큰 관리    | FCM 토큰 저장 및 갱신                | 🟢       | ⬜ 미구현 (DB 테이블만 존재) |
| FR-47 | 요일별 알림 선택      | 월~일 개별 요일 선택 UI              | 🟠       | ✅ 프론트 + 백엔드 완료 (GET/PUT /api/user/settings) |

### 2.6 설정 화면

| ID    | 요구사항         | 상세 설명                        | 우선순위 | 구현 상태                           |
| ----- | ---------------- | -------------------------------- | -------- | ----------------------------------- |
| FR-50 | 알림 시간 설정   | 시간 선택 UI (바텀시트 타임피커) | 🔴       | ✅ 프론트 + 백엔드 완료 |
| FR-51 | 푸시 알림 토글   | 알림 on/off 스위치               | 🔴       | ✅ 프론트 + 백엔드 완료 |
...
| ID    | 요구사항          | 상세 설명                     | 우선순위 | 구현 상태 |
| ----- | ----------------- | ----------------------------- | -------- | --------- |
| FR-60 | 에러 로그 수집    | 서버/클라이언트 에러 로깅     | 🔴       | ✅ Winston 구조화 로깅 구현 (파일+콘솔) |
| FR-61 | 모니터링 대시보드 | 서비스 헬스체크, API 응답시간 | 🟠       | ✅ Sentry 에러 추적 연동 (백엔드+프론트) |
| FR-62 | 사용자 통계       | DAU, 세션 시간, 뉴스 조회수   | 🟠       | ✅ 통계 API 4개 구현 (overview/popular/daily/category) |
...
| POST   | /api/auth/forgot-password | 비밀번호 재설정 인증번호 발송 | ✅ 완료 |
| POST   | /api/auth/reset-password  | 새 비밀번호 설정     | ✅ 완료   |
| POST   | /api/auth/refresh     | 토큰 갱신 (만료 토큰도 갱신, 401 자동 재시도) | ✅ 완료 |
...
| GET    | /api/user/settings | 설정 조회        | ✅ 완료   |
| PUT    | /api/user/settings | 설정 변경        | ✅ 완료   |
...
| Method | Endpoint                | 설명                         | 구현 상태 |
| ------ | ----------------------- | ---------------------------- | --------- |
| GET    | /api/stats/overview     | 전체 통계 (DAU/WAU/뉴스/인터랙션) | ✅ 완료 |
| GET    | /api/stats/popular-news | 인기 뉴스 (조회수 기준, period/limit) | ✅ 완료 |
| GET    | /api/stats/daily-active | 일별 활성 사용자 추이 (days) | ✅ 완료 |
| GET    | /api/stats/category-stats | 카테고리별 통계           | ✅ 완료 |
...
| GET    | /api/health | 표준 health check (DB+uptime) | ✅ 완료                       |
...
| Logging    | ✅ Winston 구조화 로깅 (파일+콘솔)                 |
| Monitoring | ✅ Sentry 에러 추적 (백엔드+프론트), 통계 API 4개   |
...
| #  | ID         | 작업                                        | 상세                                                  | 상태 |
| -- | ---------- | ------------------------------------------- | ----------------------------------------------------- | ---- |
| 1  | FR-42/43   | ~~설정 API 구현~~                           | GET/PUT /api/user/settings, 프론트 연동 완료           | ✅ 완료 |
| 2  | NFR-14     | ~~/health 엔드포인트~~                      | DB 연결 상태 + 서버 uptime 반환                        | ✅ 완료 |
| 3  | FR-14      | ~~비로그인 제한 로직 검증~~                 | FREE_LIMIT=3 동작 확인 완료                            | ✅ 완료 |
| 4  | FR-60      | ~~에러 로깅 시스템~~                        | Winston 구조화 로깅 (파일+콘솔), 전 라우트 적용        | ✅ 완료 |

### 9.2 우선순위 중간 (Should)

| #  | ID         | 작업                                        | 상세                                                  | 예상 난이도 |
| -- | ---------- | ------------------------------------------- | ----------------------------------------------------- | ----------- |
| 5  | FR-13      | ~~비밀번호 찾기~~                           | forgot-password + reset-password API, LoginModal forgot 모드 | ✅ 완료 |
| 6  | FR-24      | ~~Pull to refresh~~                         | NewsList 터치 제스처 구현 (리스트 뷰)                   | ✅ 완료 |
| 7  | -          | **프로덕션 배포**                           | EC2 세팅 (Frontend build + Backend + PostgreSQL)       | 높음        |
| 8  | FR-61      | ~~모니터링~~                                | Sentry 에러 추적 연동 (백엔드+프론트), ErrorBoundary 연동 | ✅ 완료 |
...
| 10 | -          | ~~토큰 갱신 API~~                           | POST /api/auth/refresh + 프론트 401 자동 재시도 인터셉터 | ✅ 완료 |
| 11 | -          | **Redis 캐시**                              | 뉴스 API 캐싱, 세션 관리                               | 중간        |
| 12 | FR-62      | ~~사용자 통계~~                             | 통계 API 4개 (overview/popular/daily-active/category-stats) | ✅ 완료 |
...
10. **프론트-백엔드 통합 완료**: 설정 API 포함 전체 기능 통합
11. **열람 로그 기록**: user_view_logs 테이블에 뉴스 조회 기록
12. **설정 API 구현 완료**: GET/PUT /api/user/settings + 프론트엔드 연동 (자동 저장)
13. **Health Check 엔드포인트**: GET /api/health (DB 연결 상태 + uptime)
14. **Winston 구조화 로깅**: 전 라우트 console.error → logger.error 전환, 파일+콘솔 출력, 요청 로깅 미들웨어
15. **비밀번호 찾기 구현**: forgot-password(인증번호 발송) + reset-password(재설정) API, LoginModal forgot 모드 추가
16. **Pull to Refresh 구현**: NewsList 리스트 뷰에서 터치 드래그로 새로고침
17. **Sentry 에러 추적 연동**: 백엔드(@sentry/node) + 프론트(@sentry/react), ErrorBoundary 연동, 글로벌 에러 핸들러 통합
18. **토큰 갱신 API 구현**: POST /api/auth/refresh (만료 토큰도 갱신), axios 인터셉터로 401 자동 재시도
19. **사용자 통계 API 구현**: overview(DAU/WAU/인터랙션), popular-news(인기뉴스), daily-active(일별 추이), category-stats(카테고리별)
