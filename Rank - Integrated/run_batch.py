...
def main():
    ...
    if not args.skip_crawl:
        # Naver News
        news_script = base_dir / "Crawling – Naver News" / "news_crawler.py"
        run_crawler(news_script, news_script.parent)
        
        # Youtube
        youtube_script = base_dir / "Crawling - Youtube" / "youtube_crawler.py"
        run_crawler(youtube_script, youtube_script.parent)
    
    # 2. Load Data
    news_file = base_dir / "Crawling – Naver News" / "news_data.json"
    youtube_file = base_dir / "Crawling - Youtube" / "youtube_data.json"
